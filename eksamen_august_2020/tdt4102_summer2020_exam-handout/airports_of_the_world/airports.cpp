...
void Airport::highlight() {
  ...
  this->set_fill_color(Color(Color::red,Color::visible));
  this->set_radius(7);
...
vector<shared_ptr<Airport>> Airports::search(string needle) const {
  ...
  needle = string_to_lower(needle);
  for(shared_ptr<Airport> elem : airport_list){
    size_t index = string_to_lower(elem->name).find(needle);
