...
shared_ptr<Airport> Explorer::lookup_airport(string code) {
  ...
  if(airports->airport_map_by_code.count(code)){
    shared_ptr<Airport> airportPtr = airports->airport_map_by_code.at(code);
    return airportPtr;
  }
  else{
    alert("No airport with that code");
...
void Explorer::do_command(Command command) {
  ...
  case HIGHLIGHT: {
    ...
    string input = in_airport.get_string();
    validate_code(input);
    shared_ptr<Airport> airportPtr = lookup_airport(input);
    airportPtr->highlight();
    highlighted.push_back(airportPtr);
...
*/
    ...
    string input = in_search_query.get_string();
    vector<shared_ptr<Airport>> result = airports->search(input);

    string output = "The search result for " + input + "\ngave the following results:\n";

    for(shared_ptr<Airport> elem : result){
      output += elem->code + ' ' + elem->name + '\n';
    }

    search_results.put(output);
  ...
  case CALCULATE_DISTANCE: {
    ...
    */
    ...
    string from = in_from_airport.get_string();
    validate_code(from);
    shared_ptr<Airport> fromPtr = lookup_airport(from);

    string dest = in_dest_airport.get_string();
    validate_code(dest);
    shared_ptr<Airport> destPtr = lookup_airport(dest);

    double distance = Util::distance(fromPtr->latitude, fromPtr->longtitude, destPtr->latitude, destPtr->longtitude);

    string output = "The distance from\n" 
                    + fromPtr->code + " to " + destPtr->code + " is\n " 
                    + to_string(distance) + " km";

    distance_results.put(output);
  ...
  case DRAW_TRIP: {
    ...
    string filename = in_trip_file.get_string();
    ifstream tripFile;
    tripFile.open(filename, ios::in);

    if(!tripFile){
      alert("No such file " + filename);
      throw FileReadError();
    }
    else{
      string textStr;

      std::getline(tripFile,textStr);

      istringstream text(textStr);
      ...
      if(codes.size() < 2){
        alert("Too few stops on the trip");
        throw FileReadError();
      }
